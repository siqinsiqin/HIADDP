    ...
    def initNetwork(self, k):
        ...
        if self.FileV == 'npy':
            ...
            lists = set_init(k, f'/{config.server}/jwj/baseExpV5/$segmentation/seg_lidc_{config.mode}_rd2/', None,
                             lists)  # 两个以上
            lists = set_init(k, self.seg_path_Lidc, None, lists)  # 单个医生标注
        ...
        if self.mode == '2d':
            ...
            train_iter = DataLoader(train_dataset, batch_size=self.train_batch_size, shuffle=True,
                                    num_workers=self.num_worker, pin_memory=False, drop_last=True)
            val_and_test_dataset = noduleSet(val_and_test_list, ['Val', '3d'], None, self.show)
            val_and_test_iter = DataLoader(val_and_test_dataset, batch_size=self.val_and_test_batch_size,
                                           num_workers=self.num_worker, pin_memory=False, shuffle=True, drop_last=True)
            test_dataset = noduleSet(val_and_test_list, ['test', '3d'], None, self.show)
            test_iter = DataLoader(test_dataset, batch_size=self.val_and_test_batch_size,
                                   num_workers=self.num_worker, pin_memory=True, shuffle=True,
                                   drop_last=True)
...
if __name__ == '__main__':
    ...
    nohup nice -n 10 python trainLIDC.py >/dev/null 2>&1 &
    ...
    model3d = ['pcamnet', 'unetr', 'asa']  # dualb
    model3d = ['dualbasa']
