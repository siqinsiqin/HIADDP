...
class evaluateBase(GC):
    ...
    def toSave(self, model, predGPU, imgGPU, mskGPU, name):
        colors = ['w', 'r', 'c', 'm', ]
        path = f'/{configs.server}/jwj/baseExpV7/predict/{self.dataset}_{model.__class__.__name__}_result/'
        ...
        if self.MetricsV == 1:
            ...
            with torch.no_grad():
                for idx, data in tqdm(enumerate(loader)):
                    ...
                    names = data.get('name', None)  # 有的 Dataset 叫 'name'，保底用 get
    ...
    def run(self, labels=None):
        self.train = False
        if labels is None and self.MetricsV == 1:  # 整体性能评估
            for model in self.model_lists:
                ...
                for loss in self.loss_lists:
                    ...
                    if configs.saveshow:
                        print('save png')
                        self.kFoldMain(5, writer, labels, )
                    else:
                        self.kFoldMain(5, writer, labels, )
                    writer(avg=True)
