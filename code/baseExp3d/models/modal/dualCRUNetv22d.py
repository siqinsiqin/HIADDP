...
from models.modal.AttentionModule2d import Attention, CASP, CASPv6
from models.modal.norm2d import Swish, unetConv2d, CResBlock, Conv2d
...
class MultiScaleDownsample(Attention):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=2, padding=0, bias=False)
        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=2, padding=1, bias=False)
...
class Upsample(nn.Module):
    def __init__(self, channel, mode='ins'):
        ...
        self.mode = mode
        if mode != 'ins':
            self.up = nn.ConvTranspose2d(channel, channel, 2, 2, padding=0, bias=False)
            self.norm = nn.BatchNorm2d(channel)
            self.swish = Swish()

    def _upsample(self, src, tar):
        return interpolate(src, size=tar.shape[2:], mode='bilinear', align_corners=True)
...
class Downsample(nn.Module):
    ...
    def __init__(self, stride, channel=None, mode='pool'):  # pool 设置在enconder
        ...
        self.mode = mode
        if mode == 'pool':
            self.maxpool = nn.MaxPool2d(stride, stride, ceil_mode=True)  # ceil_mode 向上取整
        else:
            # self.maxpool = nn.MaxPool2d(stride, stride, ceil_mode=True)
            # # self.avgpool = nn.AvgPool3d(stride, stride)
            # self.convpool = nn.Conv2d(channel, channel, kernel_size=3, padding=1, stride=stride)
            # self.softmax = nn.Softmax(dim=-1)
            #
            # # 膨胀率2
            # self.fusion = nn.Sequential(
            #     nn.Conv2d(channel * 2, channel, kernel_size=3, padding=1, stride=1, dilation=1),
...
class Enconder(nn.Module):
    def __init__(self, block_mode='3p', filters=[16, 32, 64, 128, 256], down_mode='pool'):
        ...
        if block_mode == 'u2':
            ...
            self.conv5 = unetConv2d(filters[3], filters[4], )
        elif block_mode == 'tf':
...
class dualCRUNet2d(nn.Module):

    def __init__(self, deconder='cr', depth=1):
        super(dualCRUNet2d, self).__init__()
        upmodel = 'ins'
        ...
        if deconder == 'u2':
            ...
            self.fusion4_up = unetConv2d(filters[4] + filters[3], filters[3])
            self.fusion3_up = unetConv2d(filters[3] + filters[2], filters[2])
            self.fusion2_up = unetConv2d(filters[2] + filters[1], filters[1])
            self.fusion1_up = unetConv2d(filters[1] + filters[0], filters[0])

            self.fusion4_down = unetConv2d(filters[4] + filters[3], filters[3])
            self.fusion3_down = unetConv2d(filters[3] + filters[2], filters[2])
            self.fusion2_down = unetConv2d(filters[2] + filters[1], filters[1])
            self.fusion1_down = unetConv2d(filters[1] + filters[0], filters[0])
        elif deconder == 'rs':
            self.fusion4_up = CResBlock(filters[4] + filters[3], (filters[4] + filters[3]) // 2, filters[3],
                                        mode=deconder)
            self.fusion3_up = CResBlock(filters[3] + filters[2], (filters[3] + filters[2]) // 2, filters[2],
        ...
        # todo 残差连接
        self.res4 = Conv2d(filters[4] * 2, filters[3], n=1, ks=1, padding=0)
        self.res3 = Conv2d(filters[3] * 2, filters[2], n=1, ks=1, padding=0)
        self.res2 = Conv2d(filters[2] * 2, filters[1], n=1, ks=1, padding=0)
        self.res1 = Conv2d(filters[1] * 2, filters[0], n=1, ks=1, padding=0)
    ...
    def forward(self, x):
        ...
        # todo 跳跃连接添加注意力模块
        # hu1, hx1 = torch.split(self.skip_cross_1(torch.cat((hu1, hx1), dim=1)), 32, dim=1)
        # hu2, hx2 = torch.split(self.skip_cross_2(torch.cat((hu2, hx2), dim=1)), 64, dim=1)
        # hu3, hx3 = torch.split(self.skip_cross_3(torch.cat((hu3, hx3), dim=1)), 128, dim=1)
        # hu4, hx4 = torch.split(self.skip_cross_4(torch.cat((hu4, hx4), dim=1)), 256, dim=1)

        # 第一次混合
        attnu5, attnx5 = self.neck_attn(hu5, hx5)
        fu4, fx4 = self.fusion(attnu5, attnx5, hu4, hx4, self.fusion4_up, self.fusion4_down, self.res4, self.up4_up,
                               self.up4_down)

        # 第二次融合
        attnu4, attnx4 = self.f_attn(fu4, fx4)
        ...
        # attnu3, attnx3 = checkpoint.checkpoint(self.t_attn, fu3, fx3)
        ...
        # attnu2, attnx2 = checkpoint.checkpoint(self.s_attn, fu2, fx2)
...
if __name__ == '__main__':
    ...
    var = torch.rand(1, 2, 64, 64)
    x = Variable(var).cuda()
    model = dualCRUNet2d(deconder='conv').cuda()
    ...
    macs, params = get_model_complexity_info(model, (2, 64, 64), as_strings=True,
