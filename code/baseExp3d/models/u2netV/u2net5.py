...
from models.u2netV.shuffleNet5 import RSU7, RSU6, RSU5, RSU4, RSU4F, U2baseblock
...
class Enconder(nn.Module):
    def __init__(self, block_mode='3p', filters=[16, 32, 64, 128, 256], down_mode='pool', attn=True,
        ...
        if block_mode == 'u2':
            self.conv1 = RSU5(1, filters[0] // 2, filters[0])  # , lr=True
            self.maxpool1 = Downsample(stride=2, channel=filters[0], mode=down_mode)

            self.conv2 = Block(filters[0], filters[1], block=RSU4, block_mode=block_mode)
            self.maxpool2 = Downsample(stride=2, channel=filters[1], mode=down_mode)

            self.conv3 = Block(filters[1], filters[2], block=RSU4F, block_mode=block_mode)
            self.conv3_b = u4block(filters[2], filters[3], depth=3, dirate=2, split=False, mode='ch')
    ...
    def forward(self, input):
        ...
        # v2 80.89 ，最终
        h3_b = self.conv3(h3)
        h3 = self.conv3_b(h3_b)

        return h1, h2, h3_b, h3
...
class UnetSkipDeconder(nn.Module):
    ...
    def __init__(self, sideout=False, filters=[16, 32, 64, 128, 256], deconder='conv', hamberger=False, attn=False,
        ...
        if deconder == 'conv':
            ...
            self.fusion3 = Block(filters[3] + filters[2], filters[2], block=RSU4F, block_mode=deconder)
            self.fusion2 = Block(filters[2] + filters[1], filters[1], block=RSU4, block_mode=deconder)
            self.fusion1 = Block(filters[1] + filters[0], filters[0], block=RSU5, block_mode=deconder)
        elif deconder == 'tf':
...
class UNet3PlusV5(nn.Module):
    ...
    def __init__(self, in_channels=1, frame_config=['3p', '3p', 'conv', 'conv', False], attn=True,
        ...
        # filters = [64, 128, 256, 256, 256]  # 4 stage
