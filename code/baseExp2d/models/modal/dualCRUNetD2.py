...
class dualCRUNetD2(nn.Module):

    def __init__(self, deconder='conv', depth=1):
        super(dualCRUNetD2, self).__init__()
        upmodel = 'ins'
    ...
    def forward(self, x):
        ...
        hu1, hu2 = self.up(img)
        hx1, hx2 = self.down(border)

        # 第四次融合
        # attnu2, attnx2 = self.s_attn(fu2, fx2)
        attnu2, attnx2 = channel_spilt(self.s_attn(torch.cat((hu2, hx2), dim=1)))
...
if __name__ == '__main__':
    ...
    x = Variable(var).cuda()
    model = dualCRUNetD2(deconder='conv').cuda()
