...
class Enconder(nn.Module):
    def __init__(self, inchannel=1, block_mode='3p', filters=[16, 32, 64, 128, 256], down_mode='pool'):
        super(Enconder, self).__init__()

        if block_mode == 'u2':
            self.conv1 = RSU7(inchannel, filters[0] // 2, filters[0])  # , lr=True
            ...
            self.conv1 = unetconv3d(inchannel, filters[0], )
            self.maxpool1 = Downsample(stride=2, channel=filters[0], mode=down_mode)
            ...
            self.conv1 = BasicTFBlock(inchannel, filters[0], heads=4, attn=False, trans=False)
            if not False:
                self.maxpool1 = Downsample(stride=2, channel=filters[0], mode=down_mode)
            else:
            ...
            if block_mode == 'cr':
                self.conv1 = CResBlock(inc=inchannel, midc=filters[0] // 2, outc=filters[0],
...
class dualCRUNet(nn.Module):

    def __init__(self, deconder='cr', inchannel=1):
        super(dualCRUNet, self).__init__()
        upmodel = 'ins'
        filters = [32, 64, 128, 256, 512]  # 最终实现方案 通道数
        # self.w = nn.Parameter(torch.Tensor([0.8]), requires_grad=True)
        self.up = Enconder(block_mode=deconder, filters=filters, down_mode='pool')  # 替换了enconder的基本块
        self.down = Enconder(inchannel, block_mode=deconder, filters=filters, down_mode='pool')
        ...
        self.neck_attn = CBAM(1024)
        self.f_attn = CBAM(512)
        self.t_attn = CBAM(256)
        self.s_attn = CBAM(128)
        # self.neck_attn = CASPv6(1024, depth)
    ...
    def forward(self, x):
        ...
        # sobel = x[:, 1:2, :, :, :]
        # cutn = x[:, 2:3, :, :, :]
        # border = self.w * sobel + (1 - self.w) * cutn
        ...
        attnu5, attnx5 = channel_spilt(self.neck_attn(torch.cat((hu5, hx5), dim=1)))
        ...
        attnu4, attnx4 = channel_spilt(self.f_attn(torch.cat((fu4, fx4), dim=1)))
...
if __name__ == '__main__':
    ...
    var = torch.rand(1, 3, 64, 64, 64)
    x = Variable(var).cuda()
    model = dualCRUNet(deconder='conv').cuda()
