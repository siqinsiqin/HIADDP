...
class dualCRUNetD3(nn.Module):

    def __init__(self, deconder='cr', depth=1):
        super(dualCRUNetD3, self).__init__()
        upmodel = 'ins'
        filters = [32, 64, 128]  # 最终实现方案 通道数
    ...
    def forward(self, x):
        ...
        hu1, hu2, hu3 = self.up(img)
        hx1, hx2, hx3 = self.down(border)

        # attnu3, attnx3 = self.t_attn(fu3, fx3)
        attnu3, attnx3 = channel_spilt(self.t_attn(torch.cat((hu3, hx3), dim=1)))
...
if __name__ == '__main__':
    ...
    x = Variable(var).cuda()
    model = dualCRUNetD3(deconder='conv').cuda()
