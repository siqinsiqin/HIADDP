...
class Block(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, refine_block=None, num_heads=2, mlp_ratio=4., window_size=4,
        ...
        if refine_block is not None:
            self.refine_block = refine_block(in_channel, self.mid_dim, out_channel)
        else:
...
class swinU(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, img_size=64, side=False, ape=True,
                 window_size=4, qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., patch_size=2,
                 use_checkpoint=False, ):
        super(swinU, self).__init__()
        self.side = side
        self.ape = ape

        self.en1 = RSU7(in_channel, 16, 32)
        ...
        self.en2 = RSU6(64, 32, 128)
        self.pool23 = nn.MaxPool3d(2, 2)

        self.en3 = Block(128, 256, RSU5, num_heads=2, window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop, attn_drop=attn_drop, drop_path=drop_path, use_checkpoint=use_checkpoint, depth=2)
        self.pool34 = nn.MaxPool3d(2, 2)

        self.en4 = Block(256, 320, RSU4, num_heads=2, window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop, attn_drop=attn_drop, drop_path=drop_path, use_checkpoint=use_checkpoint, depth=2)
        self.pool45 = nn.MaxPool3d(2, 2)

        self.bottle = Block(320, 320, RSU4F, num_heads=4, window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                            drop=drop, attn_drop=attn_drop, drop_path=drop_path, use_checkpoint=use_checkpoint, depth=4)

        self.de4 = Block(320 * 2, 256, RSU4, num_heads=2, window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop, attn_drop=attn_drop, drop_path=drop_path, use_checkpoint=use_checkpoint, depth=2)

        self.de3 = Block(256 * 2, 128, RSU5, num_heads=2, window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop, attn_drop=attn_drop, drop_path=drop_path, use_checkpoint=use_checkpoint, depth=2)

        self.de2 = RSU6(128 * 2, 64, 32)

        if side:
            self.de1 = RSU7(32 * 2, 32, 32)
            self.bottlesie = nn.Conv3d(320, 64, 3, 1, 1)
            self.de4side = nn.Conv3d(256, 64, 3, 1, 1)
            self.de3side = nn.Conv3d(128, 64, 3, 1, 1)
            self.de2side = nn.Conv3d(32, 64, 3, 1, 1)
            self.de1side = nn.Conv3d(32, 64, 3, 1, 1)
            ...
            self.fusion = nn.Conv3d(64 * 5, out_channel, 3, 1, 1)
        else:
            self.de1 = RSU7(32 * 2, 16, out_ch=out_channel)
