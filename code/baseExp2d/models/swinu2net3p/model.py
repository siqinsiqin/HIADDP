...
# from models.u2netV.U2netV2 import DEPTHWISECONV, DRSU5, DRSU4, DRSU4F, DRSU6, DRSU7
...
class swinu2net3plus(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, filters=None, side=False, img_size=64, window_size=4, patch_size=2,
                 ape=True, qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path_rate=0.2,
        ...
        self.en1 = DRSU7(in_channel, self.filters[0] // 2, self.filters[0], side=side)
...
class depu2net3plus(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, side=False):
        super(depu2net3plus, self).__init__()
        self.model = U2net3p5dc(in_channel, out_channel, filters=[32, 64, 128, 256, 256], side=True)
...
class swinu2net(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, img_size=64, side=False, ape=True,
        ...
        self.en2 = DRSU6(self.filter[1], self.filter[0], self.filter[1], side=side)
        self.pool23 = nn.MaxPool3d(2, 2)
        ...
        self.en3 = Block(in_channel=self.filter[1], mid_dim=self.filter[1] // 2, out_channel=self.filter[2],
                         refine_block=DRSU5, num_heads=4, side=side,
        ...
        self.en4 = Block(in_channel=self.filter[2], mid_dim=self.filter[2] // 2, out_channel=self.filter[3],
                         refine_block=DRSU4, num_heads=8, side=side,
        ...
        self.en5 = Block(in_channel=self.filter[3], mid_dim=self.filter[3] // 2, out_channel=self.filter[4],
                         refine_block=DRSU4F, num_heads=16, side=side,
                         window_size=window_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.de2 = DRSU6(self.filter[1] * 2, self.filter[1] // 2, self.filter[0], side=side)
