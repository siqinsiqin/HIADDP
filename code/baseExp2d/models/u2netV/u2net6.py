...
from models.u2netV.shuffleNet6 import RSU7, RSU6, RSU5, RSU4, RSU4F, U2baseblock
...
class Enconder(nn.Module):
    def __init__(self, block_mode='3p', filters=[16, 32, 64, 128, 256], down_mode='pool', attn=True,
                 trans=False):
        super(Enconder, self).__init__()

        if block_mode == 'u2':
            self.conv1 = RSU6(1, filters[0] // 2, filters[0])  # , lr=True
            self.maxpool1 = Downsample(stride=2, channel=filters[0], mode=down_mode)

            self.conv2 = Block(filters[0], filters[1], block=RSU5, block_mode=block_mode)
            self.maxpool2 = Downsample(stride=2, channel=filters[1], mode=down_mode)

            self.conv3 = Block(filters[1], filters[2], block=RSU4, block_mode=block_mode)
            self.maxpool3 = Downsample(stride=2, channel=filters[2], mode=down_mode)

            self.conv4 = Block(filters[2], filters[3], block=RSU4F, block_mode=block_mode)
            # self.conv4_b = Block(filters[3], filters[3], block=RSU4F, block_mode=block_mode) # cr
            self.conv4_b = u4block(filters[3], filters[3], depth=3, dirate=2, split=False, mode='ch')
            ...
            self.conv2 = BasicTFBlock(filters[0], filters[1], heads=4, attn=attn, trans=trans)
            self.maxpool2 = Downsample(stride=2, channel=filters[1], mode=down_mode)

            self.conv3 = BasicTFBlock(filters[1], filters[2], heads=8, attn=attn, trans=trans)
            self.maxpool3 = Downsample(stride=2, channel=filters[2], mode=down_mode)

            self.conv4 = BasicTFBlock(filters[2], filters[3], heads=8, attn=attn, trans=trans)
            self.maxpool4 = Downsample(stride=2, channel=filters[3], mode=down_mode)

            self.conv5 = BasicTFBlock(filters[3], filters[4], heads=16, attn=attn, trans=trans)
            ...
            if block_mode == 'cr':
                ...
                self.conv1 = CResBlock(inc=1, midc=filters[0] // 2, outc=filters[0],
                                       mode=block_mode)  # cr:none,res=block_mode
            ...
            self.conv5 = u4block(filters[3], filters[4], depth=2, dirate=2, split=False, mode='ch')
            self.conv5_b = CResBlock(filters[3], filters[3] // 2, filters[4], mode=block_mode)
    ...
    def forward(self, input):
        ...
        # v2 80.89 ，最终, cr 4 stage
        # h4_b = self.conv4(h4)
        # h4 = self.conv4_b(h4_b + h4)

        # 4-stage
        h4_b = self.conv4(h4)
        h4 = self.conv4_b(h4_b)
...
class UnetSkipDeconder(nn.Module):
    ...
    def __init__(self, sideout=False, filters=[16, 32, 64, 128, 256], deconder='conv', hamberger=False, attn=False,
                 trans=False):
        ...
        if deconder == 'conv':
            ...
            if hamberger:
                self.fusion1 = Block(filters[1], filters[0], block_mode=deconder)
            else:
                self.fusion1 = Block(filters[1] + filters[0], filters[0], block_mode=deconder)
        elif deconder == 'u2':
            # if not attn:
            #     self.fusion5 = Block(filters[4] + filters[4], filters[4], block=RSU4F, block_mode=deconder)
            # else:
            # self.fusion5_b = Block(filters[4] + filters[4], filters[4], block=RSU4F, block_mode=deconder)

            # self.fusion5 = u4block(filters[4] + filters[4], filters[4], depth=2, dirate=2, split=False, mode='ch')

            # self.fusion4 = Block(filters[4] + filters[3], filters[3], block=RSU4, block_mode=deconder)
            self.fusion3 = Block(filters[3] + filters[2], filters[2], block=RSU4, block_mode=deconder)
            self.fusion2 = Block(filters[2] + filters[1], filters[1], block=RSU5, block_mode=deconder)
            if hamberger:
                self.fusion1 = Block(filters[1], filters[0], block=RSU6, block_mode=deconder)
            else:
                self.fusion1 = Block(filters[1] + filters[0], filters[0], block=RSU6, block_mode=deconder)
        elif deconder == 'tf':
            self.fusion4 = BasicTFBlock(filters[4] + filters[3], filters[3], heads=8, attn=attn, trans=trans)
            self.fusion3 = BasicTFBlock(filters[3] + filters[2], filters[2], heads=8, attn=attn, trans=trans)
            self.fusion2 = BasicTFBlock(filters[2] + filters[1], filters[1], heads=4, attn=attn, trans=trans)
            if hamberger:
                self.fusion1 = BasicTFBlock(filters[1], filters[0], heads=4, attn=False, trans=False)
            else:
    ...
    def forward(self, features):
        ...
        # 80.61
        h1, h2, h3, h4 = features[0], features[1], features[2], features[3]
        ...
        if self.sideout:
            ...
            #
            d0 = self.outconv(channel_shuffle(torch.cat((d1, d2, d3, d4), 1), 4))
...
class SkipDeconderFactory(nn.Module):
    def __init__(self, frame_config, filters, sideout=False, atten=True, n_classes=1, attn=False,
                 trans=False):
        ...
        if name == '3p':
            ...
            if not atten:
                self.deconder = UnetSkipDeconder(sideout, filters=filters, deconder=frame_config[3], attn=attn,
                                                 trans=trans)
            elif atten:
...
class UNet3PlusV5(nn.Module):
    ...
    def __init__(self, in_channels=1, frame_config=['3p', '3p', 'conv', 'conv', False], attn=True,
        ...
        # filters = [64, 128, 256, 256, 256]  # 4 stage -cr

        # filters = [16, 32, 64, 128, 256]  # 测试
        # assert False
        # -------------Encoder--------------
        self.enconder = Enconder(block_mode=frame_config[0], filters=filters, attn=attn,
                                 trans=trans)

        # -------------Decoder--------------
        self.deconder = SkipDeconderFactory(frame_config, filters, atten=frame_config[-1], attn=attn,
                                            trans=trans)
    ...
    def forward(self, inputs):
        ...
        h1, h2, h3, h4 = self.enconder(inputs)
        d1 = self.deconder([h1, h2, h3, h4, ])
...
if __name__ == '__main__':
    ...
    x = Variable(var).cuda()
    model = UNet3PlusV5(1, frame_config=['u4', 'cat', 'conv', 'u4', False]).cuda()
