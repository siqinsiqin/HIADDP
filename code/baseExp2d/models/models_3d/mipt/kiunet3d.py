...
class kiunet3d(nn.Module):  #
    ...
    def forward(self, x):
        ...
        out1 = F.relu(
            F.interpolate(self.kencoder1(x), scale_factor=2, mode='trilinear', align_corners=True))  # Ki-Net branch
        tmp = out
        out = torch.add(out, F.interpolate(F.relu(self.intere1_1(out1)), scale_factor=0.25, mode='trilinear',
                                           align_corners=True))  # CRFB
        ...
        out1 = F.relu(F.interpolate(self.kencoder2(out1), scale_factor=2, mode='trilinear', align_corners=True))
        tmp = out
        ...
        out1 = F.relu(F.interpolate(self.kencoder3(out1), scale_factor=2, mode='trilinear', align_corners=True))
        tmp = out
        ...
        out = F.relu(F.interpolate(self.decoder1(out), scale_factor=2, mode='trilinear', align_corners=True))  # U-NET
        ...
        out = F.relu(F.interpolate(self.decoder2(out), scale_factor=2, mode='trilinear', align_corners=True))
        ...
        out = F.relu(F.interpolate(self.decoder3(out), scale_factor=2, mode='trilinear', align_corners=True))
...
if __name__ == '__main__':
    ...
    model = kiunet3d(1, 1, num_classes=1).cuda()
