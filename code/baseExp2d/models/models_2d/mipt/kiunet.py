...
class autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.relu(
            F.interpolate(self.decoder4(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))

        out = F.relu(
            F.interpolate(self.decoder5(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        # print(out.shape)
        # out = self.soft(out)
        return out
...
class unet(nn.Module):
    ...
    def forward(self, x):
        ...
        # t2 = out
        out = F.relu(
            F.interpolate(self.decoder1(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        # print(out.shape,t4.shape)
        out = torch.add(out, t4)
        out = F.relu(
            F.interpolate(self.decoder2(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        out = torch.add(out, t3)
        out = F.relu(
            F.interpolate(self.decoder3(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
...
class kinetwithsk(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(F.interpolate(self.encoder1(x), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        t1 = out
        out = F.relu(
            F.interpolate(self.encoder2(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        t2 = out
        out = F.relu(
            F.interpolate(self.encoder3(out), scale_factor=(2, 2), mode='bilinear', recompute_scale_factor=True))
        # print(out.shape)
...
class kiunet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = F.relu(
            self.enf1_bn(F.interpolate(self.encoderf1(x), scale_factor=(2, 2), mode='bilinear',
                                       align_corners=True)))  # Ki-Net branch
        tmp = out
        ...
        out1 = F.relu(
            self.enf2_bn(F.interpolate(self.encoderf2(out1), scale_factor=(2, 2), mode='bilinear', align_corners=True)))
        tmp = out
        ...
        out1 = F.relu(
            self.enf3_bn(F.interpolate(self.encoderf3(out1), scale_factor=(2, 2), mode='bilinear', align_corners=True)))
        tmp = out
        out = torch.add(out,
        ...
        out = F.relu(self.de1_bn(
            F.interpolate(self.decoder1(out), scale_factor=(2, 2), mode='bilinear', align_corners=True)))  # U-NET
        ...
        out = F.relu(
            self.de2_bn(F.interpolate(self.decoder2(out), scale_factor=(2, 2), mode='bilinear', align_corners=True)))
        ...
        out = F.relu(
            self.de3_bn(F.interpolate(self.decoder3(out), scale_factor=(2, 2), mode='bilinear', align_corners=True)))
