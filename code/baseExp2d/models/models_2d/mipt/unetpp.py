...
class UNet_Nested2d(nn.Module):

    def __init__(self, in_channels=1, n_classes=2, is_deconv=True, is_batchnorm=True, is_ds=True):
        super(UNet_Nested2d, self).__init__()
...
if __name__ == '__main__':
    ...
    x = Variable(torch.rand(3, 1, 64, 64)).cuda()
    model = UNet_Nested2d(1, 1).cuda()
    # model = smp.UnetPlusPlus(classes=1, in_channels=1).cuda()
    macs, params = get_model_complexity_info(model, (1, 64, 64), as_strings=True,
