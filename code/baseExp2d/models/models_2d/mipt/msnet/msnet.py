...
from models.models_2d.mipt.msnet.res2net import res2net50_v1b_26w_4s
...
class MSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x5_4 = self.x5_x4(abs(F.upsample(x5_dem_1, size=x4.size()[2:], mode='bilinear', align_corners=True) - x4_dem_1))
        x4_3 = self.x4_x3(abs(F.upsample(x4_dem_1, size=x3.size()[2:], mode='bilinear', align_corners=True) - x3_dem_1))
        x3_2 = self.x3_x2(abs(F.upsample(x3_dem_1, size=x2.size()[2:], mode='bilinear', align_corners=True) - x2_dem_1))
        x2_1 = self.x2_x1(abs(F.upsample(x2_dem_1, size=x1.size()[2:], mode='bilinear', align_corners=True) - x1))

        x5_4_3 = self.x5_x4_x3(abs(F.upsample(x5_4, size=x4_3.size()[2:], mode='bilinear', align_corners=True) - x4_3))
        x4_3_2 = self.x4_x3_x2(abs(F.upsample(x4_3, size=x3_2.size()[2:], mode='bilinear', align_corners=True) - x3_2))
        x3_2_1 = self.x3_x2_x1(abs(F.upsample(x3_2, size=x2_1.size()[2:], mode='bilinear', align_corners=True) - x2_1))

        x5_4_3_2 = self.x5_x4_x3_x2(
            abs(F.upsample(x5_4_3, size=x4_3_2.size()[2:], mode='bilinear', align_corners=True) - x4_3_2))
        x4_3_2_1 = self.x4_x3_x2_x1(
            abs(F.upsample(x4_3_2, size=x3_2_1.size()[2:], mode='bilinear', align_corners=True) - x3_2_1))

        x5_dem_4 = self.x5_dem_4(x5_4_3_2)
        x5_4_3_2_1 = self.x5_x4_x3_x2_x1(
            abs(F.upsample(x5_dem_4, size=x4_3_2_1.size()[2:], mode='bilinear', align_corners=True) - x4_3_2_1))
        ...
        output4 = self.output4(
            F.upsample(x5_dem_5, size=level4.size()[2:], mode='bilinear', align_corners=True) + level4)
        output3 = self.output3(
            F.upsample(output4, size=level3.size()[2:], mode='bilinear', align_corners=True) + level3)
        output2 = self.output2(
            F.upsample(output3, size=level2.size()[2:], mode='bilinear', align_corners=True) + level2)
        output1 = self.output1(
            F.upsample(output2, size=level1.size()[2:], mode='bilinear', align_corners=True) + level1)

        output = F.upsample(output1, size=input.size()[2:], mode='bilinear', align_corners=True)
        if self.training:
            return output
        return output
...
if __name__ == '__main__':
    ...
    SIZE = 64
    x = Variable(torch.rand(3, 1, SIZE, SIZE)).cuda()
    model = MSNet().cuda()
