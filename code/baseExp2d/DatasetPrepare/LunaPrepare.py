...
class FindLunaNodule(nn.Module):
    ...
    def save2D(self, name, vol, one_nodule, spacing, attrs):
        ...
        # img, msk = resample_patient(img[np.newaxis, ..., np.newaxis], msk[np.newaxis, ..., np.newaxis],
        #                             spacing, [1., 1., 1., ], force_separate_z=False)
        # img = img[:, :, 0]
        # msk = msk[:, :, 0]
        ...
        # ax[0].axis('off')
        # # ax[1].imshow(mask * img, cmap=plt.cm.gray)
        # ax[1].imshow(mask, cmap=plt.cm.gray)
        # ax[1].axis('off')
        # plt.tight_layout()
        # # plt.show()
        # os.makedirs(config.pic_path, exist_ok=True)
        # plt.savefig(config.pic_path + f'single2d.png', bbox_inches="tight", pad_inches=0)
        # plt.close()
    ...
    def save3D(self, name, vol, nodules, spacing, attrs):
        ...
        #     # plt.show()
        #     # os.makedirs(config.pic_path, exist_ok=True)
        #     plt.savefig(config.pic_path + f'pred_{t}.png', bbox_inches="tight", pad_inches=0)
        #     plt.close()
