...
class noduleSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_name = self.lists[idx].split('/')[-1]
        ...
        # 两分支 1
        # # 手工特征对比
        # if "retex" in img_name:
        #     name = img_name.replace("_retex","")
        #     prefix = re.sub(r'_\d+\.npy$', '_', Path(name).name)
        #     matches = sorted(Path("./$segmentation/seg_luna_3d/").glob(prefix + "*.npy"))

        #     a = np.load(matches[0])
        #     img_z = z_score_norm(a)
        #     img_s = img
