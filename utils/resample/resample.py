...
from utils.resample.configuration import RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD, default_num_threads
from utils.resample.cropping import get_case_identifier_from_npz, ImageCropper
...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
    ...
    if axis is not None:
        if len(axis) == 3:
            ...
            print("WARNING: axis has len 2, axis: %s, spacing: %s, target_spacing: %s" % (
                str(axis), original_spacing, target_spacing))
            do_separate_z = False
        else:
            pass
    # print(axis, do_separate_z)
    if data is not None:
    ...
    if seg is not None:
        ...
        seg_reshaped = None
    return data_reshaped[0], seg_reshaped[0]
...
def resample_data_or_seg(data, new_shape, is_seg, axis=None, order=3, do_separate_z=False, cval=0, order_z=0):
    ...
    if np.any(shape != new_shape):
        if do_separate_z:
            ...
            reshaped = []
            for c in range(data.shape[0]):
                reshaped.append(resize_fn(data[c], new_shape, order, **kwargs)[None])  # cval=cval,
...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            if scheme == "CT":
                # clip to lb and ub from luna data foreground and use foreground mn and sd from training data
                ...
                # clip to lb and ub from luna data foreground, use mn and sd form each case for normalization
                assert self.intensityproperties is not None, "ERROR: if there is a CT then we need intensity properties"
                lower_bound = self.intensityproperties[c]['percentile_00_5']
                upper_bound = self.intensityproperties[c]['percentile_99_5']
